...
// ====================================================== //
  ...
  async updateRentmyPage(subdomain: string, id: number, payload: RentMyPagePayload): Promise<unknown> {
    const result = await this.authorizedPost<unknown>(subdomain, `/pages/${id}`, payload);
    this.cache.deleteForSubdomain(subdomain);
    return result;
  }
  ...
  async getStoreNavigations(subdomain: string): Promise<{ headerLinks: NavLink[]; footerLinks: NavLink[] }> {
    ...
    try {
      ...
      const result = {
        headerLinks: fixCategoryLinks(fixProductLinks(sort((resp.result?.data || []).filter(item => item.status == 1 && item.type === 'header')))),
        footerLinks: fixCategoryLinks(fixProductLinks(sort((resp.result?.data || []).filter(item => item.status == 1 && item.type === 'footer')))),
      };
