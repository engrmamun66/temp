...
2. **Route resolution** (`DynamicRouter` catch-all) — fetches store config for the subdomain via `StoreConfigService.getRskConfigs()`, matches `req.path` against `page_slug` in the returned config, then writes `page_key` + `page_slug` into `req.context`.
